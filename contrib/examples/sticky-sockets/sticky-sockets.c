...
ssize_t magic_send(int sock, struct magic_endpoint *endpoint, void *buffer, size_t len)
{
	if (endpoint->addr.sa_family == AF_INET)
		return magic_send4(sock, endpoint, buffer, len);
	else if (endpoint->addr.sa_family == AF_INET6)
		return magic_send6(sock, endpoint, buffer, len);
	errno = EINVAL;
	return -1;
}
...
v6:
	...
	ret = magic_send(sock, &endpoint, buffer, sizeof(buffer));
	if (ret < 0) {
		perror("magic_send");
		return 1;
	}
